...
<form method="POST" action="/city/{{ $city->id }}">
	...
	<div class="row">
        ...
        <div class="input-field col s12 m6">
			<input name="shipping_cost" type="number" min="0" step="0.01" value="{{ $city->shipping_cost }}" required>
		...
		<div class="input-field col s12 m6">
			<select id="state_id" name="state_id" value="{{ $city->state_id }}" required>
				@foreach ($states as $state)
				<option id="option-{{ $state->id }}" value="{{ $state->id }}">{{ $state->name }}</option>
				@endforeach
			</select>
			<label>País</label>
...
<script type="text/javascript">
	document.getElementById('option-{{ $city->state_id }}').setAttribute("selected", "");
</script>
@endsection
